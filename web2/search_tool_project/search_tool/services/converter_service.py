...
class WordConverter:
    ...
    def convert(self, docx_path: str, cache_dir: str,
        ...
        pdf_cache_path = ConverterService.get_pdf_cache_path(docx_path, cache_dir)

        if ConverterService.is_cache_fresh(docx_path, pdf_cache_path):
        ...
        if docx_copy_dir:
            ...
            local_docx = ConverterService.get_docx_copy_path(docx_path, docx_copy_dir)
        else:
            local_docx = None
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            try:
                ...
                if ConverterService.is_encrypted_docx(local_docx):
...
class ConverterService:
    """
    Handles DOCX→PDF conversion using LibreOffice headless or Word COM.
    WordConverter context manager is preserved as a nested class for batch operations.
    """

    # Re-export WordConverter as class attribute for easy access
    ...
    def convert_docx_to_pdf(self, docx_path: str, cache_dir: str,
                            docx_copy_dir: str | None = None) -> str | None:
        """
        ...
        """
        os.makedirs(cache_dir, exist_ok=True)
        pdf_cache_path = self.get_pdf_cache_path(docx_path, cache_dir)

        if self.is_cache_fresh(docx_path, pdf_cache_path):
            return pdf_cache_path
        ...
        if docx_copy_dir:
            os.makedirs(docx_copy_dir, exist_ok=True)
            local_docx = self.get_docx_copy_path(docx_path, docx_copy_dir)
        else:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            try:
                ...
                if os.path.exists(tmp_pdf):
                    ...
                    return pdf_cache_path
                if self.is_encrypted_docx(local_docx):
    ...
    def get_pdf_path(self, original_path: str, cache_dir: str,
                     docx_copy_dir: str | None = None) -> str | None:
        """
        ...
        if ext == ".docx":
            return self.convert_docx_to_pdf(original_path, cache_dir, docx_copy_dir)
        return None
